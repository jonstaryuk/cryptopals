...
def break_repeating_key_xor(data, want_key=False, key_sizes_to_try=6, top_n=None):
    keysize_candidates = []
    ...
    if top_n is not None:
        if want_key:
            return [tup[0] for tup in plaintexts[:top_n]]
        else:
            return [tup[1] for tup in plaintexts[:top_n]]
